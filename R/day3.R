...
M_sca <- apply(M,c(1,2),median)
M_sca1 <- array(M_sca,dim=c(1,1,Y,A))
...
sel <- list(
  ...
  map_pars <- list(c(1:5,6,6,6),c(9:16))
)
...
sel2 <- list(
  ...
  initial_pars <- list(
    c(rep(0.5,4),0.5,0.5,0.5,0.5), #fixed age 5=1
    c(2,0.2)),
  map_pars <- list(c(1:5,6,6,6),c(9:10))
)
...
system.time(fit_all <- fit_wham(input_all, do.retro = TRUE, do.osa = FALSE, do.sdrep = TRUE))
fit_all$gr(fit_all$opt$par)|>round(3)
fit_all$opt
