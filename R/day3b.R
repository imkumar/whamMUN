...
Msca <- apply(M,c(1,2),median)
M_sca1 <- array(Msca,dim=c(1,1,Y,A))
...
data3M <- list(
  ...
  Msca=Msca,
  logC=data$logC,
  logCton=data$logCton,
  logCPUE.EU=data$logCPUE.EU
)
save(data3M,file='data-raw/data3M.rda')
...
M_in <- list(
  initial_MAA = array(Maa, 
                      dim = c(1,1,length(basic_info$years),
                              length(basic_info$ages)))
)
#
...
sel <- list(
  ...
  map_pars <- list(c(NA,2:5,6,6,6),c(9:16))
)
...
Mlist <- list(
  ...
  means_map=array(c(1:5,6,6,6),dim=c(1,1,A)) 
)
...
input_all <- set_NAA(input_all,NAA_re = NAA_list)

input_all <- set_F(input_all, F_opts) # this is for sel2. comment out if using sel3

input_all <- set_M(input_all, Mlist)
...
# input_all$data$use_b_prior <- 1
# input_all$random <- c(input_all$random, "log_b")
fit_all <- fit_wham(input_all, do.retro = FALSE, do.osa = FALSE, do.sdrep = TRUE)
fit_all$gr(fit_all$opt$par)
fit_all$opt
fit_all$final_gradient
