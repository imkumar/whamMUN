...
selblock_pointer_fleets <- matrix(c(rep(1,14),rep(3,21)),Y,1) #assumed 1 for fleet selectivity
...
selectivity <- list(
  model = rep("logistic",3), n_selblocks = 3,
  fix_pars = list(NULL,NULL,NULL),
  initial_pars = list(c(2,0.2),c(2,0.2),c(2,0.2))
)
...
M <- list(mean_model="estimate-M",means_map=array(1,dim=c(1,1,8)))
M <- list(mean_model="estimate-M",means_map=array(c(seq(1,5),5,5,5),dim=c(1,1,8)))
input_allM <- set_M(input_all, M)
...
fit_allM <- fit_wham(input_allM, do.retro = FALSE, do.osa = FALSE, do.sdrep = TRUE)
fit_allM$opt
fit_all$gr(fit_all$opt$par)|>round(3)

summary(fit_all$sdrep)
plot_wham_output(fit_allM)
?set_M
